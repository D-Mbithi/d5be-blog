...
- Environment variables
  - Settings are configured via `django-environ`. A `.env` file at the project root is required because `SECRET_KEY` is accessed via `env("SECRET_KEY")` with no default and will raise if missing.
  - Minimal `.env` for local/dev and tests:
    - `SECRET_KEY=any-non-empty-string`
    - `DEBUG=True`
- Database
  - SQLite is the default (`config/settings.py` → `db.sqlite3`). No extra services are required.
...
- Static & media
  ...
  - `MEDIA_ROOT` is `./media`.
  - Recipe images and user profile pictures are served from media/static depending on how the template is wired.
- Frontend (Tailwind)
  - `npm install`
...
- Important gotchas for this repo
  - `SECRET_KEY` must be available or tests will error during a settings load. Provide it via `.env` as shown above.
  ...
  - The custom user model (`apps.users.models.CustomUser`) uses email as `USERNAME_FIELD` and has no traditional username login dependency. Always use `get_user_model()` in tests.
  - Profile-related tests should use the `Profile` model from `apps.users.models` and ensure the OneToOne relation is created when needed.
- Running tests
  - Run all tests:
...
- Adding a new test (verified pattern)
  ...
  - Minimal model behavior test example:
    ...
    from apps.blog.models import Category, Post, Status
...
- Notes about `Post` model constraints
  - `slug` has `unique_for_date='publish'`. Creating draft posts with `publish=None` is fine; uniqueness is evaluated when `publish` is set. The `save()` override will set `publish=timezone.now()` when `status == Status.PUBLISHED` and `publish is None`, which is what the test exercises.
...
- `apps/`
  ...
  - `blog/`: Recipe posts, categories, and comments with `PublishedManager` and `Status` enum. `Post.save()` auto-sets publish on first transition to published.
...
- The UI is recipe-centric:
  - featured recipe hero
  - latest recipes section
  - category filters
  - chef spotlight
  - newsletter CTA
  - profile page styling should match this visual system
...
- If you create a `tests/` directory in any app, ensure there is an `__init__.py` to avoid discovery edge cases when mixing `tests.py` and `tests/`.
- Always create `Post` with `created_at` set unless you switch that field to `auto_now_add`.
- Profile pages may depend on `Profile` existing for a user; create it in setup when testing profile views.
- Recipe detail templates should not assume images exist; always provide a fallback image or placeholder.
...
- When recipe/profile features expand, update this document to describe any new required setup, media fields, or test helpers.